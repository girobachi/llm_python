...
# 検索処理（今後ここを実装・差し替え）
# -------------------------------------------------------
def search(query: str, client: MBClient) -> tuple[str, float]:
    """mem_idでcsv検索する。"""    
    client.send(f"gat like 'mem_id,{query}' limit 5")
...
# if "diary_result" not in st.session_state:
#     st.session_state.diary_result = None
#     st.session_state.diary_label = ""
#     st.session_state.diary_time = 0.0
...
# --- 日記検索ボタン ---
if st.button("日記を検索", type="secondary", disabled=(girlname == "選択してください")):
    with st.spinner("検索中..."):
        result, server_time = search_diary(girlname, shop, client)
    st.session_state.diary_result = result
    st.session_state.diary_label = f"{shop} / {girlname}"
    st.session_state.diary_time = server_time

# --- 日記表示 ---
# if st.session_state.diary_result:
#     st.subheader(f"📖 {st.session_state.diary_label}  ⏱ {st.session_state.diary_time * 1000:.0f}ms")
#     st.markdown(format_diary(st.session_state.diary_result))
...
#     with st.spinner("検索中..."):
#         result, server_time = search_diary(girlname, shop, client)
#     st.session_state.history.append((f"{shop}/{girlname}", result, server_time))

# # --- 検索入力 ---
# query = st.text_input("検索ワードを入力してください", placeholder="キーワード...", key="query_input")
...
#     if query.strip():
#         with st.spinner("検索中..."):
#             result, server_time = search(query, client)
#         st.session_state.history.append((query, result, server_time))
#     else:
...
#     for i, (q, r, server_time) in enumerate(reversed(st.session_state.history)):
#         with st.expander(f"🔍 {q}  ⏱ {server_time * 1000:.0f}ms", expanded=(i == 0)):
#             st.markdown(r)